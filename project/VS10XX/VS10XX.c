...
{								 
	//SPIx_SetSpeed(SPI_SPEED_64);//设置到低速模式 
        SPIx_SetSpeed(SPI_SPEED_32);
}
//SD卡正常工作的时候,可以高速了
void VS_SPI_SpeedHigh(void)
{						  
	SPIx_SetSpeed(SPI_SPEED_8);//设置到高速模式		 
}
//初始化VS10XX的IO口	 
void VS_Init(void)
{
	...
	//GPIOA->CRL&=0XFFF0FFFF;	//PA4 XDCS
        ...
        GPIOB->CRH&=0XF0FFFFFF;	        //PB14 XDCS
        GPIOB->CRH|=0X03000000;
	 
	//GPIOA->CRH&=0XFFF00FF0;	//PA8 XCS;PA11 RST;PA12 DQ
	//GPIOA->CRH|=0X00083003; 	 
        
        GPIOB->CRH&=0XFF000FFF;	//PB13 XCS; PB12 RST; PB11 DQ
	GPIOB->CRH|=0X00338000;
        
	//GPIOA->ODR|=(1<<4)|(1<<8)|(1<<11)|(1<<12);	//PA4,8,11,12上拉  
        
        GPIOB->ODR|=(1<<11)|(1<<12)|(1<<13)|(1<<14);	//PA4,8,11,12上拉
